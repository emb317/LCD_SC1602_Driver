...
volatile uint8_t I2CMasterBuffer[I2C_BUF_SIZE];
volatile uint8_t I2CSlaveBuffer[I2C_BUF_SIZE];
...
void I2CClearBuffer( void )
{
	I2CWriteLength = 0;
}

uint32_t I2CWrite( const uint8_t* data, uint32_t size )
{
	uint32_t i;
	for (i = 0; i < size && I2CWriteLength < I2C_BUF_SIZE; i++)
	{
		I2CMasterBuffer[I2CWriteLength] = data[i]; /* address */
		I2CWriteLength++;
	}
	return ( TRUE);
}
